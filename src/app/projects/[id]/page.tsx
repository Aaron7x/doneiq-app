...
export default function ProjectPage() {
  ...
  // --- All State Variables Grouped Together ---
  ...
  // --- Smart Mobile Default ---
  useEffect(() => {
    // If the screen is smaller than an iPad (768px), default to list view
  ...
  const fetchProjectData = async () => {
    ...
    // 2. Fetch ALL Tasks (Parents and Subtasks) for this project
    ...
    // 3. Map Subtask metrics directly to their parents
    if (allTaskData) {
      const topLevelTasks = allTaskData.filter((t: any) => !t.parent_id);
      const subTasks = allTaskData.filter((t: any) => t.parent_id);

      const enrichedTasks = topLevelTasks.map((task: any) => {
        const children = subTasks.filter((sub: any) => sub.parent_id === task.id);
        return {
          ...task,
          total_subs: children.length,
          completed_subs: children.filter((sub: any) => sub.status === "COMPLETED").length
        };
      });
  ...
  const handleQuickAddTask = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Securely fetch the active user
      const { data: { user } } = await supabase.auth.getUser();

      // 2. Attach their user.id to the new parent task
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="p-4 md:p-8 max-w-7xl mx-auto">
          ...
          {viewMode === "list" ? (
                    ...
                    <div key={task.id} onClick={() => setSelectedTaskId(task.id)} className="flex items-center gap-4 p-4 bg-gray-800/50 hover:bg-gray-800 transition-colors rounded-xl border border-gray-700/50 cursor-pointer">
                      ...
                      <div className="flex flex-col flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {task.task_type === "bug" && <Bug className="h-4 w-4 text-red-500 opacity-80" />}
                        </div>
                      </div>
                      
                      {/* --- LIST VIEW SUBTASK PILL --- */}
                      {(task.total_subs ?? 0) > 0 && (
                        <span className={`text-[10px] font-bold tracking-widest flex items-center gap-1.5 px-3 py-1.5 rounded-lg border ${task.completed_subs === task.total_subs ? 'bg-green-500/10 text-green-500 border-green-500/30' : 'bg-gray-950 text-gray-400 border-gray-700'}`}>
                          <CheckSquare className="h-3 w-3" />
                          {task.completed_subs}/{task.total_subs}
                        </span>
                      )}

                      <span className="ml-4 text-[10px] font-bold tracking-widest bg-gray-950 border border-gray-700 text-gray-400 px-3 py-1.5 rounded-lg shrink-0">
            ...
            <KanbanBoard initialTasks={tasks} onTaskClick={setSelectedTaskId} />
          )}
        </div>
      </main>
